...
namespace Core.UI
{
    public class VirtueView : MonoBehaviour
    {
        ...
        {
            DivisionsCount = _gameSettings.VirtueLevels;
        }

        private void Start()
        {
            _rectTransform = GetComponent<RectTransform>();
            SeparateBarOnLevels(DivisionsCount + 1);
            SetFillAmount(Random.Range(0.4f, 0.9f));

            // Create final division with ultimate wonder
            float lastDivision = 1f - (1f / (DivisionsCount + 1));
